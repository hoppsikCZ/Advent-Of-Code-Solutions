...
bool is_sue_valid(std::unordered_map<std::string, int> conditions,
    ...
    for (auto &[key, val] : sue) {
        ...
        if (key == "cats" || key == "trees") {
            key_type = KeyType::Greater;
        } else if (key == "pomeranians" || key == "goldfish") {
            key_type = KeyType::Lower;
        } else {
            key_type = KeyType::Equal;
        }

        if (key_type == KeyType::Equal && conditions[key] != val ||
            key_type == KeyType::Lower && conditions[key] <= val ||
            key_type == KeyType::Greater && conditions[key] >= val) {
            return false;
        }
    }
