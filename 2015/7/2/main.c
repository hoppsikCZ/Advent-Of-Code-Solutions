...
void reset_wires(struct wire *const wire_map, size_t size) {
...
int main(int argc, char *argv[]) {
    ...
    reset_wires(wire_map, count);

    struct wire *wire_b = find_key(wire_map, count, "b");
    wire_b->val = result;
    wire_b->calculated = true;

    result = eval_wire(wire_map, count, find_key(wire_map, count, "a"));
